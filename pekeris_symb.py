...
def get_modes(wg: Waveguide, h: float, branch: Branch,
              n: int = 4000, margin: float = 1e-4) -> List[Mode]:
    """Find all wavenumbers k_j(h) for `branch` and package them as Modes."""
    ks = wg.find_wavenumbers(h, branch, n=n, margin=margin)
...
def _F(c, x):
    ...
    return np.where(np.abs(c) < 1e-12, 0.0,
...
def S_sc(a, b, D):
    ...
    return (a * np.cosh(a * D) * np.sin(b * D) - b * np.sinh(a * D) * np.cos(b * D)) / (a ** 2 + b ** 2)


def T_fun(a, b, D):
    """int_0^D u cosh(au) sin(bu) du."""
    num = ((b ** 2 - a ** 2) * np.cosh(a * D) * np.sin(b * D)
...
def R_fun(a, b, D):
    ...
    num = ((b ** 2 - a ** 2) * np.sinh(a * D) * np.cos(b * D)
...
def _U1(l: Mode, j: Mode) -> float:
    ...
    h = l.h
    return -(l.k * l.kp) / (2 * l.wg.rho1 * l.kappa1) * (
...
def coupling_coefficient(l: Mode, j: Mode) -> float:
    ...
    if l.branch == "water" and j.branch == "water":
        u2 = (j.A / rho2) * (l.dA * J_ss(l.g2, j.g2, D) + l.A * l.g2p * K_cs(l.g2, j.g2, D))
    elif l.branch == "bottom" and j.branch == "bottom":
        u2 = (j.C / rho2) * (l.dC * L_ss(l.x2, j.x2, D) + l.C * l.x2p * M_cs(l.x2, j.x2, D))
    elif l.branch == "water" and j.branch == "bottom":
        u2 = -(j.C / rho2) * (l.dA * S_sc(l.g2, j.x2, D) + l.A * l.g2p * T_fun(l.g2, j.x2, D))
    elif l.branch == "bottom" and j.branch == "water":
        u2 = -(j.A / rho2) * (l.dC * S_sc(j.g2, l.x2, D) + l.C * l.x2p * R_fun(j.g2, l.x2, D))
    else:
...
def coupling_matrix(modes_l: List[Mode], modes_j: List[Mode]) -> np.ndarray:
    """
    ...
    """
    V = np.empty((len(modes_l), len(modes_j)))
    for p, l in enumerate(modes_l):
        for q, j in enumerate(modes_j):
...
def coupling_coefficient_numeric(wg: Waveguide, branch_l: Branch, idx_l: int,
                                  branch_j: Branch, idx_j: int, h: float,
    ...
    def dphihat_dh(branch, idx, hh, z):
        return (phihat(branch, idx, hh + eps, z) - phihat(branch, idx, hh - eps, z)) / (2 * eps)
...
def plot_dispersion(wg: Waveguide, h: float, branch: Branch,
                     ax=None, n: int = 2000, margin: float = 1e-4):
    ...
    if branch == "water":
        kmin, kmax, G = wg.omega / wg.c2 * (1 + margin), wg.omega / wg.c1 * (1 - margin), wg.Gw
    else:
        kmin, kmax, G = wg.omega / wg.c2 * margin, wg.omega / wg.c2 * (1 - margin), wg.Gb
    ...
    ax.plot(roots, np.zeros_like(roots), "o", color="crimson", zorder=5, label="$k_j$ roots")
...
def plot_modes(wg: Waveguide, modes: List[Mode], ax=None,
               n_z: int = 800, normalize: bool = True, offset: float = 0.0):
    """
    ...
    for i, m in enumerate(modes):
        ...
        ax.plot(phi + i * offset, z, style, color=color,
                label=f"{m.branch} #{m.index}  $k={m.k:.4f}$")

    ax.axhline(modes[0].h, color="gray", lw=1, ls=":", label=f"interface $h={modes[0].h:g}$")
...
def plot_coupling_matrix(V: np.ndarray, labels_l=None, labels_j=None, ax=None,
                          cmap: str = "RdBu_r"):
...
def main():
    ...
    wg = Waveguide(c1=1500.0, c2=1800.0, rho1=1.0, rho2=1.5,
                    H=400, omega=2 * np.pi * 50.0)  # 80 Hz source
    h = 100.0

    water_modes = get_modes(wg, h, "water")
    bottom_modes = get_modes(wg, h, "bottom")
    print(f"Found {len(water_modes)} water-borne modes, "
    ...
    V = coupling_matrix(all_modes, all_modes)
